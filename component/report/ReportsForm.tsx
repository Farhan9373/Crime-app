...
export function ReportForm({ onComplete }: ReportFormProps) {
  ...
  const generateReportId = useCallback(() => {
    ...
    return array[0].toString(36).padStart(8, "0"); // Base36 gives shorter strings
  }, []);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 px-4 sm:px-6 max-w-2xl mx-auto">
      {/* Emergency Type Selection */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <button
          type="button"
          onClick={() => setFormData((prev) => ({ ...prev, incidentType: "EMERGENCY" }))}
          className={`w-full p-4 sm:p-6 rounded-2xl border-2 transition-all duration-200 ${
      ...
      <div className="relative group">
        ...
        <label
          ...
          className="block w-full p-6 sm:p-8 border-2 border-dashed border-zinc-700 rounded-2xl 
                     hover:border-sky-500/50 hover:bg-sky-500/5 transition-all duration-200
                     cursor-pointer text-center"
        >
          {image ? (
            <div className="space-y-4">
              <div className="w-full h-40 sm:h-52 relative rounded-lg overflow-hidden">
            ...
            <div className="space-y-4">
              <svg className="mx-auto h-10 w-10 text-zinc-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
              <p className="text-sm text-zinc-400">Drop an image or click to upload</p>
            </div>
          )}
        </label>
        {isAnalyzing && (
      ...
      <div>
        ...
        <select
          ...
          className="w-full rounded-xl bg-zinc-900/50 border border-zinc-800 px-4 py-3.5 text-white 
                     transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-sky-500/40"
          required
        >
      ...
      <button
        ...
        className="w-full relative group overflow-hidden rounded-xl bg-gradient-to-br from-sky-500 to-blue-600 
                   px-4 py-3.5 text-sm font-medium text-white shadow-lg transition-all duration-200 
                   hover:from-sky-400 hover:to-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
      >
